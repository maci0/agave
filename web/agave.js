...
const arrayBufferFromSource = (source) => {
    if (source instanceof ArrayBuffer) {
        return source;
    }
    ...
    async init(source = default_wasm_url, signal) {
        ...
        const importObject = {
            ...
            wasi_snapshot_preview1: {
                ...
                fd_close: () => 0,
                fd_seek: () => 0,
                proc_exit: (code) => { throw new AgaveError('wasm_invalid', `Process exit: ${String(code)}`); },
                environ_get: () => 0,
                environ_sizes_get: () => 0,
                clock_time_get: () => 0,
                random_get: (ptr, len) => {
