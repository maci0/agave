...
const retry_delay_ns: u64 = 1 * std.time.ns_per_s;
...
pub fn printUsage() void {
    ...
    stdout.writeAll(
        \\USAGE:
        \\  agave pull <org/repo> [OPTIONS]
        \\
        \\Download a GGUF model from HuggingFace Hub.
        \\
        \\OPTIONS:
        \\  -Q, --quant <QUANT>  Select quantization (e.g. Q4_K_M, Q8_0)
...
pub fn parseArgs(args_iter: *std.process.ArgIterator) PullError!?PullArgs {
    ...
    // Read optional HF token from environment.
    ...
    while (args_iter.next()) |arg| {
        ...
        } else if (std.mem.eql(u8, arg, "--quant") or std.mem.eql(u8, arg, "-Q")) {
            result.quant = args_iter.next() orelse {
    ...
    if (std.mem.indexOfScalar(u8, result.repo, '/') == null) {
...
pub fn listGgufFiles(allocator: Allocator, repo: []const u8, token: ?[]const u8) (PullError || Allocator.Error)!ListResult {
    ...
    for (siblings) |sibling| {
        ...
        if (std.mem.endsWith(u8, rfilename.string, ".gguf")) {
            gguf_count += 1;
        }
    }
    ...
    for (siblings) |sibling| {
        ...
        const rfilename = sibling.object.get("rfilename") orelse continue;
        if (rfilename != .string) continue;
        if (!std.mem.endsWith(u8, rfilename.string, ".gguf")) continue;
...
fn httpGet(allocator: Allocator, url: []const u8, token: ?[]const u8) (PullError || Allocator.Error)![]u8 {
    ...
    // Build extra headers for auth token.
    ...
    const extra_headers: []const std.http.Header = if (auth_value) |av| blk: {
        extra_headers_buf[0] = .{ .name = "Authorization", .value = av };
...
fn replaceSlashes(allocator: Allocator, input: []const u8) Allocator.Error![]u8 {
    ...
    const out_len = input.len + slash_count; // each '/' becomes '--' (1 extra char)
...
fn createAgaveSymlink(allocator: Allocator, repo: []const u8, snapshot_dir: []const u8) void {
    ...
    // Remove existing symlink if present (ignore FileNotFound).
    std.fs.cwd().deleteFile(link_path) catch |err| switch (err) {
        error.FileNotFound => {},
        else => eprint("Warning: could not remove old symlink: {}\n", .{err}),
    };

    std.posix.symlink(snapshot_dir, link_path) catch |err| {
...
fn downloadFile(
    ...
    while (attempt < max_retries) : (attempt += 1) {
        ...
        downloadFileOnce(allocator, url, blob_path, token) catch |err| {
...
fn downloadFileOnce(
    ...
    var eh_idx: usize = 0;
    if (auth_value) |av| {
        extra_headers_storage[eh_idx] = .{ .name = "Authorization", .value = av };
        eh_idx += 1;
    }
    if (range_value) |rv| {
        extra_headers_storage[eh_idx] = .{ .name = "Range", .value = rv };
        eh_idx += 1;
    }

    var req = client.request(.GET, uri, .{
        .extra_headers = extra_headers_storage[0..eh_idx],
        .keep_alive = false,
    }) catch |err| {
    ...
    const file = if (start_offset > 0)
        std.fs.cwd().openFile(blob_path, .{ .mode = .write_only }) catch
            return PullError.DownloadFailed
    else
        std.fs.cwd().createFile(blob_path, .{}) catch
            return PullError.DownloadFailed;
    defer file.close();
...
/// 2. Select the best file (or print list and exit)
/// 3. Download the file to the HuggingFace cache layout
/// 4. Create snapshot and ref symlinks
/// 5. Create agave convenience symlink
/// 6. Print the final model path
pub fn pullModel(allocator: Allocator, args: PullArgs) (PullError || Allocator.Error)!void {
    ...
    if (std.mem.endsWith(u8, selected.filename, ".gguf")) {
        if (std.fs.cwd().openFile(blob_path, .{})) |f| {
            ...
            if (n < 4 or !std.mem.eql(u8, &magic, "GGUF")) {
                eprint("Warning: downloaded file does not have valid GGUF header — may be corrupt\n", .{});
    ...
    // Remove existing symlink if present (ignore FileNotFound).
    std.fs.cwd().deleteFile(snapshot_link) catch |err| switch (err) {
        error.FileNotFound => {},
        else => eprint("Warning: could not remove old snapshot symlink: {}\n", .{err}),
    };
    std.posix.symlink(relative_blob, snapshot_link) catch |err| {
