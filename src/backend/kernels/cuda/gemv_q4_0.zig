...
export fn gemv_q4_0_kernel(x: [*]const f32, w: [*]const u8, y: [*]f32, n: u32, k: u32) callconv(.nvptx_device) void {
...
test "constants valid" {
    comptime std.debug.assert(q4_0_block_size > 0);
    comptime std.debug.assert(q4_0_group_size > 0);
    comptime std.debug.assert(nr > 0);
}

test "fuzz: gemv_q4_0 functions" {
