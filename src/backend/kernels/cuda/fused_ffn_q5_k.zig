...
const gemv_q5k = @import("gemv_q5_k.zig");
...
export fn fused_ffn_gate_up_silu_q5_k_kernel(
    ...
    while (blk < blocks_per_row) : (blk += bdim) {
        ...
        gate_sum += gemv_q5k.q5kBlockDot(x, w_gate + row * row_bytes + blk * bytes_per_block, n_embd, base_col);
        up_sum += gemv_q5k.q5kBlockDot(x, w_up + row * row_bytes + blk * bytes_per_block, n_embd, base_col);
    }
