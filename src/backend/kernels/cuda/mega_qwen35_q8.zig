...
//! For Qwen 3.5 0.8B: max(1536, 4096) = 4096 blocks.
//!
...
// All dispatched blocks must be co-resident for this to work.
// On CUDA, this requires cooperative launch or guaranteed occupancy.
...
/// Uses a global atomic counter; thread 0 of each block increments,
/// then spins until all blocks have arrived. syncthreads() ensures
/// all threads in each block see the barrier completion.
...
// Same as fused_ffn_q8_0.zig. Computes dot(scale * quants, x) for one
// 32-element quantization block.
...
// Each stage processes one forward-pass operation across all blocks.
// Blocks beyond the stage's output dimension are idle.

/// RMS norm: output[i] = input[i] * weight[i] * rsqrt(mean(x^2) + eps).
/// Multi-block cooperative: all blocks contribute to sum-of-squares via
...
fn rmsNormStage(
    ...
    // Grid sync: wait for all blocks to contribute
    gridSync(sync_ctr);
...
/// GEMV Q8_0: y[row] = dot(W[row,:], x). One block per output row.
/// Blocks beyond n_out are idle.
fn gemvQ8Stage(
    x: [*]const f32,
    w: [*]const u8,
    y: [*]f32,
...
/// Distributed across all blocks (each block handles a subset of pairs).
fn ropeStage(
    x: [*]f32,
    n_heads: u32,
    head_dim: u32,
    ...
    // Distribute pairs across all blocks
    var idx = bid * bdim + tid;
    while (idx < total) : (idx += n_blocks * bdim) {
        const h = idx / half_rd;
        const d = idx % half_rd;

        // freq = exp(-ln(theta) * 2d / rope_dim), computed via log2 + exp2
...
export fn megakernel_qwen35_q8_kernel(
    ...
    while (li < n_layers) : (li += 1) {
        ...
        siluMulStage(ff_gate, ff_up, n_ff);
        gridSync(&sync_ctrs[sync_idx % n_sync_slots]);
        gridSyncReset(&sync_ctrs[sync_idx % n_sync_slots]);
        sync_idx += 1;
