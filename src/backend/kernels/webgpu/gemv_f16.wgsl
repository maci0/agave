// F16 matrix-vector multiply: y[row] = dot(f16_to_f32(W[row, :]), x)
// F16 format: 2 bytes per element, packed as u16 pairs in u32 words.
...
fn main(
    ...
    // Each row has k f16 elements = k/2 u32 words
    ...
    while (word_idx < words_per_row) {
        ...
        // unpack2x16float: low 16 bits = .x, high 16 bits = .y
