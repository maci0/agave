...
export fn gemv_q5_k_kernel(x: [*]const f32, w: [*]const u8, y: [*]f32, n: u32, k: u32) callconv(.kernel) void {
    ...
    while (blk < blocks_per_row) : (blk += bdim) {
        ...
        var bmax: f32 = 0.0;
        const check_end = @min(base_col + q5_k_group_size, k);
...
test "constants valid" {
    comptime std.debug.assert(q5_k_block_size > 0);
    comptime std.debug.assert(q5_k_group_size > 0);
    comptime std.debug.assert(q5_k_high_bit_int > 0);
