//! Qwen4-Exp (Qwen3.8-Flash-Next, qwen4_exp / Qwen4ExpForConditionalGeneration).
//! Hybrid: 36× Gated DeltaNet (linear_attention) + 12× Qwen Sparse Attention
//! (full_attention, every 4th layer). MoE 512 experts top-10 + shared (640),
//! ngram PLE at layer 2, Gated Residual (Hyper-Connections, 4 streams), QSA
//! sparse indexer, MTP-1, vision 27-block ViT. NVFP4 is the primary quant
//! (group 16, E2M1 + FP8 E4M3 scales) for MoE experts; attention/linear SSM
//! stay BF16 per quantization_config.ignore.
//!
...
//! experts (group 16, 9B/16). Current agave GEMV is correct; a future
//! cutlass-style batched GEMM can adopt the perm/strided layout for
//! SM120/121 if profiling shows a win.
...
pub const Qwen4ExpModel = struct {
    ...
    fn mropeCoords(self: *const Qwen4ExpModel, seq_idx: usize) [3]usize {
    ...
    fn applyLmRope(self: *Qwen4ExpModel, x: [*]f32, seq_idx: usize, n_heads: usize) void {
    ...
    pub fn init(allocator: Allocator, f: Format, be: Backend, ctx_size: u32, kv_type_k: kv_quant.KvQuantType, kv_type_v: kv_quant.KvQuantType, tiered_cache: ?*TieredKvCache) !Qwen4ExpModel {
        ...
        const arch = f.getMetaStr("general.architecture") orelse "qwen4_exp";
        // Qwen4-Exp Flash-Next: 48 layers, 2560 hidden, 24/2 heads, hd 256,
        // 512 experts top-10, 640 MoE dim, HC 4×320, PLE at layer 2.
        // Prefer explicit text_config values; fall back to generic arch prefix.
        ...
        // text_config overrides (SafeTensors HF: inside text_config — only
        // apply when actually a HF dir, not bare GGUF with unrelated keys).
        if (f.is_safetensors and (f.getMetaU32("hidden_size") != null or f.getMetaU32("num_hidden_layers") != null)) {
        ...
        // MoE configuration — qwen4_exp: 512 experts top-10, 640 dim.
        // Prefer HF top-level keys (num_experts etc.) which are authoritative
        // for SafeTensors qwen4_exp; fall back to arch-prefixed and tensor probe.
        const expert_count_meta = f.getMetaU32("num_experts") orelse
            f.getArchU32(arch, "expert_count") orelse blk: {
            if (f.layerTensor(0, "ffn_gate_exps.weight")) |t| {
                const n_from_tensor: u32 = if (t.n_dims >= 1) @intCast(t.dims[0]) else 512;
        ...
        if (expert_count_meta) |ec| {
            if (ec > 0) {
                self.is_moe = true;
                self.n_experts = ec;
                self.n_experts_active = f.getMetaU32("num_experts_per_tok") orelse
                    f.getArchU32(arch, "expert_used_count") orelse 10;
                self.expert_ff_dim = f.getMetaU32("moe_intermediate_size") orelse
                    f.getArchU32(arch, "expert_feed_forward_length") orelse 640;
        ...
        if (self.n_head_kv == 0 or self.n_head % self.n_head_kv != 0) {
            std.log.err("qwen4_exp: n_head ({d}) not divisible by n_head_kv ({d})", .{ self.n_head, self.n_head_kv });
            return error.MissingTensor;
        }
        if (self.ssm_dt_rank == 0 or self.ssm_d_inner % self.ssm_dt_rank != 0) {
            std.log.err("qwen4_exp: ssm_d_inner ({d}) not divisible by ssm_dt_rank ({d})", .{ self.ssm_d_inner, self.ssm_dt_rank });
            return error.MissingTensor;
        }
        if (self.rope_dim > self.head_dim) {
            std.log.err("qwen4_exp: rope_dim ({d}) exceeds head_dim ({d})", .{ self.rope_dim, self.head_dim });
            return error.MissingTensor;
        }
        if (self.rope_dim % 2 != 0) {
            std.log.err("qwen4_exp: rope_dim ({d}) is not even", .{self.rope_dim});
            return error.MissingTensor;
        }
        if (self.is_moe) {
            if (self.n_experts_active > self.n_experts) {
                std.log.err("qwen4_exp: n_experts_active ({d}) exceeds n_experts ({d})", .{ self.n_experts_active, self.n_experts });
                return error.MissingTensor;
            }
            if (self.n_experts_active > max_active_experts) {
                std.log.err("qwen4_exp: n_experts_active ({d}) exceeds max_active_experts ({d})", .{ self.n_experts_active, max_active_experts });
                return error.MissingTensor;
            }
        }
    ...
    fn ssmConvChannels(self: *const Qwen4ExpModel) usize {
    ...
    fn warmNormCache(self: *Qwen4ExpModel) void {
    ...
    fn embLookup(self: *Qwen4ExpModel, tok: u32) !void {
    ...
    fn isFullAttn(self: *const Qwen4ExpModel, layer: u32) bool {
    ...
    inline fn layerVType(self: *const Qwen4ExpModel, li: u32) kv_quant.KvQuantType {
    ...
    fn syncProfile(self: *Qwen4ExpModel) void {
        if (self.perf.enabled) self.be.sync();
    }

    /// Helper: get KV cache byte slices for a layer from the first paged/tiered block.
    fn getLayerKvView(self: *Qwen4ExpModel, layer: usize) struct { keys: []u8, values: []u8 } {
    ...
    fn getPagedKvView(self: *Qwen4ExpModel, layer: usize) PagedKvView {
    ...
    fn isMultiBlock(self: *Qwen4ExpModel, layer: usize) bool {
    ...
    fn doGemv(self: *Qwen4ExpModel, x: [*]const f32, t: TensorInfo, y: [*]f32, n: usize, k: usize) void {
    ...
    fn shardColumnWeight(self: *const Qwen4ExpModel, t: TensorInfo, n_total: usize, k: usize) TensorInfo {
    ...
    fn shardRowWeight(self: *Qwen4ExpModel, t: TensorInfo, n: usize, k_total: usize, shard_buf: []u8) TensorInfo {
        if (self.tp_degree <= 1) return t;
    ...
    fn makeOp(self: *Qwen4ExpModel, t: TensorInfo, y: [*]f32, n: usize, k: usize) backend_mod.GemvOp {
    ...
    fn doGemvBatch2(self: *Qwen4ExpModel, x: [*]const f32, t0: TensorInfo, y0: [*]f32, n0: usize, t1: TensorInfo, y1: [*]f32, n1: usize, k: usize) void {
    ...
    fn doGemvBatch3(self: *Qwen4ExpModel, x: [*]const f32, t0: TensorInfo, y0: [*]f32, n0: usize, t1: TensorInfo, y1: [*]f32, n1: usize, t2: TensorInfo, y2: [*]f32, n2: usize, k: usize) void {
    ...
    fn doGemm(self: *Qwen4ExpModel, x: [*]const f32, t: TensorInfo, y: [*]f32, n_tok: usize, n_out: usize, n_in: usize) void {
        if (t.dtype == .mlx_q) {
            for (0..n_tok) |i| {
    ...
    fn doGemvExpert(self: *Qwen4ExpModel, x: [*]const f32, exp_t: TensorInfo, ei: usize, stride: usize, y: [*]f32, n: usize, k: usize) void {
    ...
    fn applyAttnGate(self: *Qwen4ExpModel, attn: [*]f32, gate: [*]f32, n: usize) void {
    ...
    fn fullAttnLayer(self: *Qwen4ExpModel, li: u32, fuse_ffn_residual: bool) !void {
        const e: usize = self.n_embd;
    ...
    // ---- DeltaNet SSM layer ----
    // Reference: llama.cpp build_layer_attn_linear in src/models/qwen4_exp.cpp
    /// DeltaNet SSM layer. When `fuse_ffn_residual` is true, the pre-attention
    /// norm fuses the previous layer's deferred FFN residual (hidden2) into
    /// hidden via addRmsNorm instead of a separate add + rmsNorm.
    fn deltaNetLayer(self: *Qwen4ExpModel, li: u32, fuse_ffn_residual: bool) !void {
        const e: usize = self.n_embd;
        ...
        if (num_v_heads > max_ssm_v_heads) @panic("qwen4_exp: num_v_heads exceeds max_ssm_v_heads");
    ...
    fn ffnCompute(self: *Qwen4ExpModel, li: u32) !void {
        const e: usize = self.n_embd;
    ...
    fn mlpLayer(self: *Qwen4ExpModel, li: u32, defer_residual: bool) !void {
        const e: usize = self.n_embd;
        const ff: usize = if (self.tp_degree > 1) self.n_ff / self.tp_degree else self.n_ff;
    ...
    pub fn getBlockTable(self: *Qwen4ExpModel) []const u32 {
    ...
    pub fn mtpForward(self: *Qwen4ExpModel, token_id: u32, depth: u32) !u32 {
    ...
    pub fn getMtpLogits(self: *Qwen4ExpModel) []f32 {
        return self.mtp_logits_buf;
    }

    /// Reset MTP KV cache position for speculation rejection.
    pub fn resetMtpCache(self: *Qwen4ExpModel) void {
    ...
    fn prefillChunk(self: *Qwen4ExpModel, token_ids: []const u32, base_pos: u32) !void {
    ...
    fn prefillAttention(self: *Qwen4ExpModel, li: u32, n_tok: usize) !void {
        const e: usize = self.n_embd;
        const nh: usize = self.n_head;
        const nkv: usize = self.n_head_kv;
        const hd: usize = self.head_dim;
    ...
    fn prefillFeedForward(self: *Qwen4ExpModel, li: u32, n_tok: usize) !void {
        const e: usize = self.n_embd;
        const ff: usize = self.n_ff;
    ...
    pub fn sendKvCache(self: *Qwen4ExpModel, transport: *TransportMod.Transport) !void {
    ...
    pub fn recvKvCache(self: *Qwen4ExpModel, transport: *TransportMod.Transport) !void {
        const kvd = self.paged_cache.kv_dim;
        const bs = self.paged_cache.block_size;
        const elems_per_block = @as(usize, bs) * kvd;
    ...
    pub fn saveSsmState(self: *const Qwen4ExpModel, allocator: std.mem.Allocator) ![]u8 {
    ...
    pub fn restoreSsmState(self: *Qwen4ExpModel, snapshot: []const u8) void {
        var pos: usize = 0;
        for (0..self.n_layers) |i| {
            const conv_bytes = self.conv_states[i].len * @sizeOf(f32);
    ...
    pub fn setCaptureLayers(self: *Qwen4ExpModel, ids: []const u32, capacity: usize) !void {
        ...
        for (ids) |id| {
            if (@as(i64, id) <= prev or id >= self.n_layers) {
                std.log.err("qwen4_exp: capture layer ids must be sorted, unique, < n_layers (got {any})", .{ids});
                return error.MissingTensor;
            }
            prev = id;
        }
        ...
        std.log.info("qwen4_exp: capturing hidden states at layers {any} (window {d}) for DFlash2", .{ ids, capacity });
    }

    fn clearCapture(self: *Qwen4ExpModel) void {
        const a = self.allocator;
    ...
    pub fn captureBegin(self: *const Qwen4ExpModel) usize {
        return self.kv_seq_len -| self.capture_count;
    }

    /// One past the newest capture position.
    pub fn captureEnd(self: *const Qwen4ExpModel) usize {
    ...
    pub fn readFeatures(self: *const Qwen4ExpModel, pos: usize, dst: []f32) void {
        const e = self.n_embd;
    ...
    fn captureSlot(self: *const Qwen4ExpModel, l: u32) ?usize {
...
test "attnGateIsSilu parses DeltaNet gated-norm activation" {
    try std.testing.expect(Qwen4ExpModel.attnGateIsSilu("swish"));
    try std.testing.expect(Qwen4ExpModel.attnGateIsSilu("silu"));
    try std.testing.expect(!Qwen4ExpModel.attnGateIsSilu("sigmoid"));
    try std.testing.expect(!Qwen4ExpModel.attnGateIsSilu(""));
}
...
test "rmsNormPlusOne correctness" {
    ...
    Qwen4ExpModel.rmsNormPlusOne(&input, &output, &weight, 4, 1e-6);
...
test "model vtable compiles" {
    // Verify that Qwen4ExpModel implements the Model vtable interface
    const M = model_mod.Model;
    _ = M.from;
    // Verify the struct has the expected public API surface
    try std.testing.expect(@hasDecl(Qwen4ExpModel, "forward"));
    try std.testing.expect(@hasDecl(Qwen4ExpModel, "prefill"));
    try std.testing.expect(@hasDecl(Qwen4ExpModel, "resetCache"));
    try std.testing.expect(@hasDecl(Qwen4ExpModel, "cancel"));
    try std.testing.expect(@hasDecl(Qwen4ExpModel, "model"));
}
...
test "fuzz: all qwen4_exp functions" {
    try std.testing.fuzz({}, struct {
        fn f(_: void, smith: *std.testing.Smith) !void {
            ...
            // ── 5. rmsNormPlusOne: static function, fuzz-safe ──
            {
                ...
                Qwen4ExpModel.rmsNormPlusOne(&input, &output, &weight, n, eps);
                for (0..n) |i| {
            ...
            comptime {
                ...
                _ = &Qwen4ExpModel.model;
                _ = &Qwen4ExpModel.init;
                _ = &Qwen4ExpModel.deinit;
                _ = &Qwen4ExpModel.cancel;
                _ = &Qwen4ExpModel.getBlockTable;
                _ = &Qwen4ExpModel.mtpForward;
                _ = &Qwen4ExpModel.getMtpLogits;
                _ = &Qwen4ExpModel.resetMtpCache;
                _ = &Qwen4ExpModel.forward;
                _ = &Qwen4ExpModel.prefill;
                _ = &Qwen4ExpModel.sendKvCache;
                _ = &Qwen4ExpModel.recvKvCache;
                _ = &Qwen4ExpModel.saveSsmState;
                _ = &Qwen4ExpModel.restoreSsmState;
                _ = &Qwen4ExpModel.resetCache;
                // Verify the type itself
                _ = @sizeOf(Qwen4ExpModel);
            }
        }
    }.f, .{});
}
