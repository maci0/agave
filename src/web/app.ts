...
interface PendingStreamPaint {
  el: HTMLElement;
  content: string;
}

interface SidebarWithTrap extends HTMLElement {
  _trapFocus?: ((e: KeyboardEvent) => void) | null;
}

interface ModalWithTrap extends HTMLElement {
  _trapFocus?: ((e: KeyboardEvent) => void) | null;
}
...
const chat = qs('#chat');
const inp = qs<HTMLTextAreaElement>('#msg');
...
if (savedSystemPrompt) {qs<HTMLTextAreaElement>('#system-prompt').value = savedSystemPrompt;}
qs<HTMLTextAreaElement>('#system-prompt').addEventListener('input', function(this: HTMLTextAreaElement) {
...
tempEl.addEventListener('input', function(this: HTMLInputElement) {
...
topPEl.addEventListener('input', function(this: HTMLInputElement) {
...
function loadImageFile(file: File, label: string) {
  ...
  reader.addEventListener('load', function(ev: ProgressEvent<FileReader>) {
    ...
    pendingImage = result;
    qs<HTMLImageElement>('#img-thumb').src = pendingImage;
...
function onImageSelect(e: Event) {
  ...
  if (!loadImageFile(file, 'Image attached')) {input.value = '';}
}

function removeImage() {
  pendingImage = null;
  const thumb = qs<HTMLImageElement>('#img-thumb');
  thumb.removeAttribute('src');
  thumb.src = '';
  qs('#img-preview').style.display = 'none';
  qs<HTMLInputElement>('#img-input').value = '';
...
inp.addEventListener('keydown', function(e) {
  ...
  if (e.key === 'Enter' && !e.shiftKey && !e.isComposing) {
    e.preventDefault();
    qs<HTMLFormElement>('#chat-form').requestSubmit();
  }
});
...
chatForm.addEventListener('dragleave', function(e) {
  ...
  if (to instanceof Node && chatForm.contains(to)) {return;}
...
chatForm.addEventListener('drop', function(e) {
  ...
  if (file?.type.startsWith('image/')) {loadImageFile(file, 'Image dropped');}
});

document.addEventListener('keydown', function(e) {
  if (e.key === 'Escape') {
    if (qs('#info-modal').classList.contains('show')) {hideInfo();}
    else if (qs('#sidebar').classList.contains('open')) {toggleSidebar();}
    else if (qs('#settings-panel').classList.contains('open')) {toggleSettings();}
...
function setStreaming(s: boolean) {
  ...
  inp.disabled = s;
  const imgBtn = qs<HTMLButtonElement>('#img-btn');
...
function getSystemParam() {
  const sp = qs<HTMLTextAreaElement>('#system-prompt').value.trim();
...
function clearSystemPrompt() {
  const el = qs<HTMLTextAreaElement>('#system-prompt');
  el.value = '';
...
function updateCtxBadge(modelData: ModelRecord) {
  const badge = qs('#ctx-badge');
  if (!modelData) {return;}
  const used = modelData.kv_seq_len ?? 0;
  const max = modelData.ctx_size ?? 0;
  if (max <= 0) {return;}
  const fmtCtx = function(n: number) { return n >= 1024 ? `${fmtInt(Math.round(n / 1024))}K` : fmtInt(n); };
...
function exportConv() {
  ...
  for (const w of msgs) {
    ...
    const msgHtml = msgEl as HTMLElement;
...
function addUser(text: string, imageSrc?: string | null) {
  const emptyEl = qs('#empty'); if (emptyEl) {emptyEl.remove();}
...
let hljsLoad: Promise<boolean> | null = null;

/** Fetch highlight.js and its theme on first code block. Copy/lang chrome does not wait. */
function loadHighlightJs(): Promise<boolean> {
  if (hljs) {return Promise.resolve(true);}
  if (hljsLoad) {return hljsLoad;}
...
function decorateCodeBlock(b: Element) {
...
function highlightCodeBlocks(el: HTMLElement) {
  const blocks = el.querySelectorAll('pre code');
  if (blocks.length === 0) {return;}
  for (const b of blocks) {decorateCodeBlock(b);}
  ...
  if (hljs) {apply(); return;}
  loadHighlightJs().then(function(ok) { if (ok) {apply();} });
}
...
function markdownToHtml(dc: string) {
  if (!marked) {return escapeHtmlText(dc);}
  if (!markedConfigured) {
...
function demoteHeadings(el: HTMLElement) {
  for (const h of el.querySelectorAll('h1, h2, h3, h4, h5, h6')) {
    ...
    const nh = document.createElement(`h${next as 2 | 3 | 4 | 5 | 6}`);
...
function hardenLinks(el: HTMLElement) {
...
function attachCopyButton(el: HTMLElement, content: string) {
  const cb = document.createElement('button'); cb.type = 'button'; cb.className = 'msg-copy'; cb.textContent = 'Copy';
...
function renderFinal(el: HTMLElement, content: string) {
  ...
  if (DOMPurify) {
    ...
    const container = document.createElement('div'); container.innerHTML = sanitized;
...
function mkStat(label: string, val: string, unit?: string) {
  ...
  sp.append(v);
  if (unit) { sp.append(document.createTextNode(` ${unit}`)); }
  return sp;
}

function addStats(el: HTMLElement, s: StreamStats) {
  ...
  if (s.pfMs && s.pfMs !== '0') {d.append(mkStat('TTFT ', fmtInt(s.pfMs), 'ms'));}
...
function onSubmit(e: Event) {
  ...
  if (text.startsWith('/')) {runCommand(text);} else {sendMessage(text);}
  return false;
}
...
function clearChat() {
  ...
  if (hasMsgs && !confirm('Clear this conversation?')) {return;} // oxlint-disable-line no-alert -- native confirmation dialog is intentional UX
...
function setMobileSidebarModal(on: boolean) {
  const sb = qs<SidebarWithTrap>('#sidebar');
  const main = qs('.main');
  const hdr = qs('header');
  ...
  if (on) {
    ...
    sb._trapFocus = function(e: KeyboardEvent) {
      if (e.key !== 'Tab') {return;}
      ...
      if (!(first instanceof HTMLElement) || !last) {return;}
      if (e.shiftKey) {
...
function deleteConv(id: string) {
  if (!confirm('Delete this conversation?')) {return;} // oxlint-disable-line no-alert -- native confirmation dialog is intentional UX
...
function setDialogBackdropInert(on: boolean) {
...
// oxlint-disable-next-line no-unused-vars -- called from HTML onclick
function showInfo() {
  const m = qs<ModalWithTrap>('#info-modal'); m.classList.add('show');
  ...
  if (dlg instanceof HTMLElement) {dlg.focus();}
  else {
  ...
  m._trapFocus = function(e: KeyboardEvent) {
    ...
    if (focusable.length === 0) {
      if (dlg instanceof HTMLElement) { e.preventDefault(); dlg.focus(); }
      return;
    }
    const first = focusable[0];
    const last = lastFocusable(focusable);
...
function hideInfo() {
  const m = qs<ModalWithTrap>('#info-modal'); m.classList.remove('show');
