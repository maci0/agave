...
fetch('/v1/models').then(function(r) { return r.json(); }).then(function(d) {
  if (d.data?.[0]) {
    ...
    const badge = document.querySelector('#model-name');
...
function setOfflineBadge() {
  ...
  btn.addEventListener('click', function() {
    ...
    fetch('/v1/models').then(function(r) { return r.json(); }).then(function(d) {
      const el = document.querySelector('#model-name');
      if (d.data?.[0]) {
        modelName = d.data[0].id;
        backendName = d.data[0].backend ?? '';
...
if (savedSystemPrompt) {document.querySelector('#system-prompt').value = savedSystemPrompt;}
...
const maxTokEl = document.querySelector('#max-tokens');
...
maxTokEl.addEventListener('input', function() {
  ...
  const errEl = document.querySelector('#max-tokens-error');
...
function removeImage() {
  pendingImage = null;
  const thumb = document.querySelector('#img-thumb');
...
function setStreaming(s) {
  ...
  const tc = document.querySelector('#toks-counter');
  if (s) {
...
function getSamplingParams() {
  ...
  let maxTok = Number.parseInt(document.querySelector('#max-tokens').value, 10);
...
function getSystemParam() {
  const sp = document.querySelector('#system-prompt').value.trim();
...
function clearSystemPrompt() {
  const el = document.querySelector('#system-prompt');
  el.value = '';
...
function updateCtxBadge(modelData) {
  const badge = document.querySelector('#ctx-badge');
  if (!modelData) {return;}
...
function announceToSR(text) {
  const sr = document.querySelector('#sr-announce');
...
function markdownToHtml(dc) {
  ...
  if (!globalThis.marked) {return escapeHtmlText(dc);}
  try {
    return globalThis.marked.parse(dc);
...
function renderFinal(el, content) {
  ...
  if (globalThis.DOMPurify) {
    const sanitized = globalThis.DOMPurify.sanitize(parsed, {ADD_TAGS: ['details', 'summary']});
...
async function streamResponse(body, errLabel, url) {
  ...
  try {
    ...
    if (error.name === 'AbortError') { renderContent(el, content || '*Stopped*', true); addRegenBtn(el); }
      ...
      const errMsg = `${errLabel}: ${error.message}`;
...
function showEmpty() {
  ...
  for (const t of ['/help for commands', 'Enter to send']) {
    ...
    if (isHelp) { s.type = 'button'; s.addEventListener('click', function() { runCommand('/help'); }); }
    hintsEl.append(s);
  }
...
// oxlint-disable-next-line no-unused-vars -- called from HTML onclick
function showInfo() {
  const m = document.querySelector('#info-modal'); m.classList.add('show');
...
function hideInfo() {
  const m = document.querySelector('#info-modal'); m.classList.remove('show');
