...
agave model.gguf --serve --max-batch-size 16   # Batch width (admission one-at-a-time until per-request paged KV is wired)
agave model.gguf --serve --rate-limit-rpm 60 --rate-limit-tpm 100000  # Token-bucket rate limits
```

```bash
# SSD expert streaming
agave model.gguf --ssd-streaming "prompt"                        # Stream experts from SSD
```
...
**Key files:** [build.zig](build.zig), [src/backend/backend.zig](src/backend/backend.zig) (dispatcher), [src/backend/webgpu.zig](src/backend/webgpu.zig) (WebGPU backend), [src/backend/accelerate.zig](src/backend/accelerate.zig) (Apple Accelerate.framework BLAS), [src/models/model.zig](src/models/model.zig) (model vtable), [src/models/dflash2.zig](src/models/dflash2.zig) (DFlash2 block-diffusion drafter: KV injection, grouped dynamic conv, candidate path selector), [src/spec/dflash2.zig](src/spec/dflash2.zig) (DFlash2 pure algorithm kernels), [src/models/llama4.zig](src/models/llama4.zig) (Llama 4: iRoPE, chunked attention, MoE), [src/models/deepseek4.zig](src/models/deepseek4.zig) (DeepSeek V4: hyper connections, MLA, CSA/HCA compressors, LID, dedicated CpuBackend bypass for Metal), [src/models/qwen4exp.zig](src/models/qwen4exp.zig) (Qwen 3.8 Flash-Next GGUF: HC, GDN sigmoid gate, QSA, n-gram PLE, 512-expert MoE), [src/models/qwen4_exp.zig](src/models/qwen4_exp.zig) (Qwen4-Exp SafeTensors NVFP4 + ngram SSD), [src/ngram_cache.zig](src/ngram_cache.zig) (PLE ngram shard LRU), [src/backend/kernels/metal/ds4.metal](src/backend/kernels/metal/ds4.metal) (10 DS4 Metal kernels), [src/backend/kernels/metal/ds4_fused.metal](src/backend/kernels/metal/ds4_fused.metal) (fused attention megakernel), [src/ops/quant.zig](src/ops/quant.zig), [src/ops/attention.zig](src/ops/attention.zig), [src/ops/ssm.zig](src/ops/ssm.zig), [src/chat_template.zig](src/chat_template.zig), [src/recipe.zig](src/recipe.zig), [src/backend/mega_compose.zig](src/backend/mega_compose.zig) (megakernel composer), [src/backend/megakernel.zig](src/backend/megakernel.zig) (weight offsets), [src/term.zig](src/term.zig) (terminal I/O, key parsing, display width), [src/cli.zig](src/cli.zig) (CLI argument parser), [src/spec/ddtree.zig](src/spec/ddtree.zig) (DDTree speculative decoding), [src/spec/spec_decode.zig](src/spec/spec_decode.zig) (spec decode orchestrator), [src/spec/dspark.zig](src/spec/dspark.zig) (DSpark: confidence-scheduled verification, Markov/RNN head, SPS scheduler), [src/spec/ngram.zig](src/spec/ngram.zig) (n-gram speculative decoding), [src/grammar.zig](src/grammar.zig) (GBNF parser, JSON schema→grammar, constrained decoding), [src/server/server.zig](src/server/server.zig) (HTTP server, tool calling, vision API, prompt prefix caching), [src/server/json.zig](src/server/json.zig) (JSON parsing, sampling params, tool/vision extraction), [src/ops/math.zig](src/ops/math.zig) (sampling, penalties, logprobs), [src/ops/kv_quant.zig](src/ops/kv_quant.zig) (KV cache quantization: TurboQuant/PlanarQuant/IsoQuant/RotorQuant), [src/wasm_entry.zig](src/wasm_entry.zig) (WASM browser entry point), [src/kvcache/manager.zig](src/kvcache/manager.zig) (paged KV cache, PagedKvView), [src/kvcache/tiered.zig](src/kvcache/tiered.zig) (tiered KV cache, TransferCallback), [src/parallel/transport.zig](src/parallel/transport.zig) (TCP + shm + NCCL transport), [src/parallel/peer_discovery.zig](src/parallel/peer_discovery.zig) (UDP peer discovery), [src/devices/discovery.zig](src/devices/discovery.zig) (GPU device enumeration), [src/pull.zig](src/pull.zig) (HF Hub model download), [src/spec/pflash.zig](src/spec/pflash.zig) (PFlash speculative decoding), [src/lora.zig](src/lora.zig) (LoRA adapter merge at load time), [src/thread_pool.zig](src/thread_pool.zig) (centralized parallel CPU dispatch), [src/steering.zig](src/steering.zig) (directional steering vectors), [src/expert_cache.zig](src/expert_cache.zig) (LRU expert cache for SSD streaming), [src/image_tokens.zig](src/image_tokens.zig) (multimodal image token handling)

**Docs:** [KERNELS.md](docs/KERNELS.md) (kernel status per backend), [MODELS.md](docs/MODELS.md) (model params), [MEGAKERNEL.md](docs/MEGAKERNEL.md) (megakernel system), [CONTRIBUTING.md](docs/CONTRIBUTING.md) (how to add backends/models/quants), [DOCUMENTATION.md](docs/DOCUMENTATION.md) (tutorials index), [API.md](docs/API.md) (HTTP API reference), [ARCHITECTURE.md](docs/ARCHITECTURE.md) (project structure, inference pipeline), [BENCHMARKS.md](docs/BENCHMARKS.md) (performance data), [TEST_MATRIX.md](docs/TEST_MATRIX.md) (model × backend test status), [PARALLELISM.md](docs/PARALLELISM.md) (distributed inference), [TODO.md](docs/TODO.md) (roadmap)
...
- Model toggles: `-Denable-<model>=false` (gemma3, gemma4, diffusion-gemma, qwen35, qwen4exp, qwen4-exp, gpt-oss, nemotron-h, nemotron-nano, glm4, deepseek4, llama4).
- Backend toggles: `-Denable-<backend>=false` (cpu, cuda, metal, rocm, vulkan, webgpu). `-Denable-debug=true` for debug builds.
...
Supports 6 backends (Metal, CUDA, Vulkan, ROCm, WebGPU, CPU), 12 model architectures (Gemma3/4, DiffusionGemma, Qwen3.5/Nex-N2-Pro, Qwen 3.8 Flash-Next GGUF, Qwen4-Exp NVFP4, GPT-OSS, Nemotron-H/Nano, GLM-4, DeepSeek V4, Llama 4), and extensive quantization (Q2-Q8, FP8, bf16, NVFP4, MXFP4, MLX, TurboQuant KV). Megakernel system with composable building blocks for fused GPU dispatch. CLI and HTTP server interfaces. Built with Zig 0.16.0. Zero external dependencies.
