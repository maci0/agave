...
|---|---------|--------|-------|
| 1 | Tensor/Pipeline parallelism | Working | 6 modes: local TP, distributed TP, distributed PP, hybrid TP+PP, disaggregated prefill/decode, dual-GPU same-node. Transports: TCP (cross-node), POSIX shm (same-node zero-copy), NCCL (infrastructure, blocked by context corruption on GB10). `--transport auto/tcp/shm/nccl`, `--device N`, `--peers`. Verified: CUDA↔CUDA PP 8.9 tok/s, CUDA↔CUDA TP 4.7 tok/s (9B Q8_0), CUDA↔CPU, Vulkan↔Vulkan (dGPU+iGPU), heterogeneous x86_64+aarch64 |
