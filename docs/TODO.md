...
| 13 | oxlint not yet enforced on the two large web sources | Medium | Open. `bunx oxlint` is green on `web/agave.ts`, `web/agave.js` and `src/web/globals.d.ts`, but `src/web/app.ts` (275 errors) and `web/shell.ts` (95) are in `.oxlintrc.json` `ignorePatterns` until they are migrated. The bulk is `strict-boolean-expressions` on DOM presence checks, the `no-unsafe-*` family on untyped `fetch` JSON, `require-safety-comment-for-type-assertion` on DOM casts, and `func-style`/`no-use-before-define` ordering. Doing it properly means typing the SSE and `/v1/*` response bodies at their boundary rather than casting at each use, and there is no runtime test for the chat UI, so migrate one file at a time and check with `bunx tsc --noEmit -p src/web/tsconfig.json` plus a manual load of the served UI. Remove each path from `ignorePatterns` as it lands; the count may fall, never rise. |
