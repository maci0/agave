...
All of this is wired generically, not per-model. `ModelStorage.initFromArch()` (`src/models/model.zig`) takes a `tp_rank`/`tp_degree` pair and checks `@hasField(M, "tp_rank")` at comptime before setting them; `setPpConfig()` does the same for `pp_rank`/`pp_degree`/`pp_transport`. Architectures that declare those fields today: Qwen 3.5 (`src/models/qwen35.zig`) and DeepSeek V4 Flash (`src/models/deepseek4.zig`). Qwen shards dense FFN columns; DeepSeek shards routed MoE experts. Transport is a 2-rank pair, so the CLI accepts `--tp 2` and `--pp 2` and rejects degrees above 2.
...
- **`--tp` and `--pp` are 2-rank only.** The CLI accepts `--tp 2` / `--pp 2` and rejects anything above 2 (`transport is a 2-rank pair only`, exit code 2). Qwen 3.5 FFN TP and DeepSeek V4 expert-parallel TP both go through `Transport.allReduceAdd`. `--pp` and `--disagg` use the same pair. See [PARALLELISM.md](../PARALLELISM.md) for invocations.
