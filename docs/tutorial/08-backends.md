...
**Browser / WASM** (`wasm_entry.zig`): A separate WASM entry point exports `agave_init`, `agave_generate`, `agave_get_output`, `agave_last_error`, `agave_free`, `agave_alloc`, and `agave_dealloc` for calling from JavaScript (`AgaveEngine` / `AgaveError` in `web/agave.ts`). Model data is passed as a byte buffer from JS (no file I/O); `agave_free` owns that buffer. The WASM build uses CPU backend only (no GPU in wasm32 freestanding). Note: full forward-pass inference is currently blocked by a Zig 0.16 + LLVM 21 wasm32 codegen bug; model init, GGUF parsing, and tokenization work.
