...
True megakernels execute an entire transformer layer (or multiple layers) in a single GPU dispatch. They use composable building blocks from `mega_common.metal` (730 lines, 18 primitives) with atomic counter grid sync for cross-threadgroup coordination.
...
**Pipeline/kernel counts** (from backend constants in source): Metal `n_pipelines = 88` (+ runtime-composed megakernels), CUDA `n_kernels = 61`, ROCm `n_kernels = 49`, Vulkan `n_pipelines = 49`, WebGPU ~48 WGSL shaders. Total megakernel code: ~4,923 lines across 16 files plus ~1,050 lines in `mega_compose.zig` (composable generator).
