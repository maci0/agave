...
| q5_0 | Native (SIMD) | Native | Native | Missing | Native | Native |
...
| Vulkan | `src/backend/kernels/vulkan/` | `silu.comp`, `silu_mul.comp`, `gelu.comp`, `gelu_mul.comp`, `add.comp`, `add_rms_norm.comp`, `add_scaled.comp`, `mul.comp`, `rms_norm.comp`, `rms_norm_multi.comp`, `softmax.comp`, `l2_norm.comp`, `rope.comp`, `sigmoid_mul.comp`, `deinterleave.comp`, `split_qgate.comp`, `embedding.comp`, `conv1d.comp`, `deltanet_recurrence.comp`, `sdpa.comp`, `sdpa_turbo.comp`, `sdpa_paged.comp`, `sdpa_tree.comp`, `gemv_{f32,q8_0,q4_0,q4_1,q5_0,bf16,f16,q4_k,q5_k,q6_k,fp8_e4m3,fp8_e5m2,gptq,mlx_q4,nvfp4_st,mxfp4_st,t_q8_0}.comp` (+compiled `.spv`) |
...
**Pipeline/kernel counts**: Metal 70+ pipelines (+ 1 runtime-composed), CUDA 51 kernels, ROCm 42 kernels, Vulkan 40 shaders, WebGPU 39 shaders. Total megakernel code: ~4,166 lines across 12 files plus ~780 lines in `mega_compose.zig` (composable generator).
