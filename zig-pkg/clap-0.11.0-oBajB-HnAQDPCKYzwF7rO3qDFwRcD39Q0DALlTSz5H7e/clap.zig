...
pub const Diagnostic = struct {
    ...
    pub fn reportToFile(diag: Diagnostic, file: std.Io.File, err: anyerror) !void {
        // Zig 0.16: File.writer requires Io context unavailable here.
        // Format error message directly and write via raw C write.
        var buf: [1024]u8 = undefined;
        var longest = diag.name.longest();
        if (longest.kind == .positional)
            longest.name = diag.arg;
        const msg = std.fmt.bufPrint(&buf, "Error: {s} for argument '{s}{s}'\n", .{
            @errorName(err),
            longest.kind.prefix(),
            longest.name,
        }) catch "Error while parsing arguments\n";
        _ = std.c.write(file.handle, msg.ptr, msg.len);
    }
};
...
pub const ParseOptions = struct {
    ...
    /// OS args vector (Zig 0.16: ArgIterator removed, pass args from process.Init).
    os_args: ?[]const [*:0]const u8 = null,
};

/// Same as `parseEx` but uses OS args (Zig 0.16: pass os_args via ParseOptions).
pub fn parse(
    comptime Id: type,
    ...
    var iter = args.NullTermIterator{ .args = opt.os_args orelse &.{} };
...
fn Arguments(
    ...
    var field_names: [fields_len][:0]const u8 = undefined;
    var field_types: [fields_len]type = undefined;
